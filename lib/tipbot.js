...
var TipBot = function(slack, BLOCKTRAIL_APIKEY, BLOCKTRAIL_APISECRET, SECRET, TESTNET, OPTIONS) {
    ...
    self.AMOUNT_REGEX = new RegExp('(\\d+\\.?\\d*) *(Satoshis?|BTC|' + self.CURRENCIES.join("|") + ')', "i");
        ...
        self.getPriceRates(function(err, rates) {
            ...
            if (!rate) {
                ...
                var text = value.toFixed(2) + " " + currency + " " +
                    "(" + blocktrail.toBTC(newValue) + " BTC at " + rate.rate.toFixed(2) + " " + currency + " / BTC)";

                return cb(newValue, rate.rate, currency, value, text);
            }
        });
    } else {
        return cb(value, null, null, null, blocktrail.toBTC(value) + " BTC");
    }
};

TipBot.prototype.tellHelp = function(channel) {
    //{srce, target} = channel.name.split(" ")
    //do look up COMMANDS['en']
    console.log(channel.send(
        "*PAYBOT COMMANDS* \n" +
        "Please note all withdrawals subject to a 5% fee - thanks!_\n" +
        " - *balance*\t\task the bot for your current balance; _@paybot what is my balance_ \n" +
        " - *send*\t\t\t\ttell the bot to send coins to someone; _@paybot send 0.1 BTC to @someone_ \n" +
        "\t\t\t\t\t\t\tor; _@paybot give 4 USD to @someone_ \n" +
        "\t\t\t\t\t\t\t_aliases: give_ \n" +
        " - *receive*\t\ttell the bot to request coins from to someone; _@paybot receive 0.1 BTC from @someone_ \n" +
        "\t\t\t\t\t\t\t_aliases: demand, ask, deserve, get, give me, owes me_ \n" +
        " - *deposit*\t\task the bot for a deposit address; _@paybot let me deposit!_ \n" +
        " - *withdraw*\ttell the bot to withdraw to an address; _@paybot withdraw 1 BTC to 1dice8EMZmqKvrGE4Qc9bUFf9PX3xaYDp!_ \n" +
        " - *currencies*\task the bot for a list of supported currencies; _@paybot currencies PLX!_ \n" +
        " - *price*\task the bot for the bitcoin price in a particular currency; _@paybot price in USD!_ \n" +
        " - *convert*\task the bot to convert between a particular currency and bitcoin (or vice versa); _@paybot 1 USD to EUR!_ \n" +
        "\t\t\t\t\t\t\tor; _@paybot 0.03 BTC to GBP_ \n" +
        " - Read more about Bitcoin here: https://bitcoin.org/en/faq_ \n"
    ));
};
...
TipBot.prototype.onMessage = function(channel, member, message) {
    ...
    if (message.match(/(all|every(one)?s?) ?balances?/i)) {
        ...
        channel.send("Retrieving all balances... might take a while depending on the amount of users!");
    ...
    if (message.match(/balance/i)) {
        if (channel.is_channel) {
            channel.send("I don't think you really want me to reveal your balance to the public channel, " + user.handle + " :/");
        }
    ...
    if (message.match(/deposit/i)) {
        user.tellDepositAddress(self.slack.getDMByUserId(user.id));
    ...
    /*
     * WITHDRAW
     * extend to subtract fee - where are the funds now? whoever hosts the coins.
     */
    if (message.match(/withdraw/i)) {
        ...
        self.normalizeValue(amount[1], amount[2], function(value, rate, originalCurrency, originalValue, valueText) {
            if (rate === false) {
                ...
                dm.send("The final amount you will receive (less TX fees): " + 0.95 * value);
                ...
                self.triggers.push(new Trigger(
                    ...
                    function(channel, message, _user, userMatches) {
                        ...
                        if (channel.id == dm.id && _user.id == user.id && message.match(/(OK|yes|fine|sure)/i)) {
                            user.withdraw(self.slack.getDMByUserId(user.id), 0.95 * value, address);
                            trigger.destroy();
        ...
        if (amount && matches) {
            ...
            self.normalizeValue(amount[1], amount[2], function(value, rate, originalCurrency, originalValue, valueText) {
                if (rate === false) {
                    ...
                    if (value < MIN_TIP) {
                        channel.send(user.handle + ": the minimum amount you can pay is " + blocktrail.toBTC(MIN_TIP) + " BTC");
     ...
     */
    if (userMatches.length > 1) {
        channel.send("Sorry " + user.handle + " but you're including too many people!");

        return;
    } else if (userMatches.length === 1) {
        channel.send("Sorry " + user.handle + " but you mentioned somebody but I could  not understand what you wanted!");
                    ...
                    self.normalizeValue(amount[1], amount[2], function(value, rate, originalCurrency, originalValue, valueText) {
                        if (rate === false) {
                            ...
                            if (currency == 'BTC') {
                                ...
                                channel.send(blocktrail.toBTC(value) + " BTC (" + rate.toFixed(2) + " " + amount[2] + " / BTC)");
                            } else if (amount[2] == 'BTC') {
                                self.getPriceRates(function(err, rates) {
                                    ...
                                    if (!rate) {
                                        ...
                                        var newValue = blocktrail.toBTC(value) * rate.rate;
                                ...
                                self.getPriceRates(function(err, rates) {
                                    ...
                                    if (!currencyRate) {
                                        ...
                                        var newValue = blocktrail.toBTC(value) * currencyRate.rate;
        ...
        if (currency) {
            ...
            self.getPriceRates(function(err, rates) {
                ...
                if (!rate) {
                    ...
                    channel.send("1.0 BTC is " + rate.rate.toFixed(2) + " " + currency);
                }
            });
