...
User.prototype.tellDepositAddress = function(channel) {
    var self = this;
    ...
    self.getWallet(function(wallet) {
        wallet.getNewAddress(function(err, address) {
            ...
            //QR Code here 
            var qrCode = qrImage.imageSync(`bitcoin:${address}?label=paybot?message=deposit`, { type: 'png', margin: 1, size: 5 });
            channel.send(self.handle + " Please deposit to this address; " + address);
            channel.send(qrCode);
        });
    });
};
...
/*
TODO Add Maximum sendable figure to pay given balance.
            ...
            self.getWallet(function(wallet) {
                wallet.getBalance(function(err, confirmed, unconfirmed) {
                    if (err) {
                        ...
                        cb(new Error("Sorry " + self.handle + " you can't send your full balance, need to account for the Bitcoin fees ..."));
                    } else if (confirmed >= value) {
                        cb(null, true);
                    } else if (confirmed + unconfirmed >= value) {
